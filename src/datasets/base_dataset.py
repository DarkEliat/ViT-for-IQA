...
class BaseDataset(TorchDataset):
    def __init__(self, config):
        ...
        for column_name in self.labels.select_dtypes(include='object').columns:
            self.labels[column_name] = (self.labels[column_name].str.strip()
                                                                .str.replace("\\\\", "/", regex=False)
                                                                .str.replace("\\", "/", regex=False))

        if self.labels.isnull().any().any():
    ...
    def __getitem__(self, index):
        ...
        reference_image_path = self._reference_images_map.get_file_path(reference_image_name)
        distorted_image_path = self._distorted_images_map.get_file_path(distorted_image_name)
